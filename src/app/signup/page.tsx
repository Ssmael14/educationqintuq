...
export default function SignupPage() {
  ...
  async function handleSignup() {
    ...
    // Si no hay error, Better Auth redirige via callbackURL
  }
    ...
    <div className="min-h-[100dvh] flex flex-col md:items-center md:justify-center bg-gradient-to-b from-peach-soft to-cream md:bg-cream">
      ...
      <main className="flex-1 md:flex-none w-full max-w-md mx-auto px-6 pt-16 pb-6 md:py-12 flex flex-col">
        ...
        >
          ...
          {/* ✅ Checkbox controlado — necesario para bloquear Google también */}
          <label className="flex items-start gap-2 mt-4 text-xs text-ink-soft font-bold leading-relaxed cursor-pointer">
            <input
              type="checkbox"
              checked={accepted}
            ...
            <span>
              Acepto los{" "}
              <Link href="/terms" className="text-sky">
                Términos
              </Link>{" "}
              y la{" "}
          ...
          <button
            ...
            className="btn-chunky w-full py-3 rounded-2xl bg-white border-2 border-cream font-bold text-ink flex items-center justify-center gap-2 hover:border-ink/20 disabled:opacity-50"
          >
            {loadingGoogle ? (
              "Conectando..."
            ) : (
